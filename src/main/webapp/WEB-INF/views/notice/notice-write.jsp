            ...
            <div class="nav">
                ...
                <div class="contents">
                    ...
                    <div class="notice_write">
                        ...
                        <form action="<c:url value='/notice/write'/>" method="post" onsubmit="return confirmSubmit()">
                            <c:if test="${modify=='1'}">
                            ...
                            <div class="notice_button">
                                <c:choose>
                                    <c:when test="${modify == '1'}">
                                        <input type="submit" formaction="<c:url value="/notice/modify"/>" value="수정" >
                                    </c:when>
                                    <c:otherwise>
                                        <button type="submit">등록</button>
                                    </c:otherwise>
                                </c:choose>
                                <button type="button" OnClick="if(confirm('취소하시겠습니까?')){ location.href ='<c:url value="/notice/list"/>${sc.optionQueryString}';}">취소</button>
                            </div>
                        </form>
                    </div>
                </div>
